...
def visualize_boxes_and_labels_on_image_array(
      ...
      if scores is None:
        ...
        if not skip_scores:
          if not display_str:
            ...
            if 'Fish' in box_to_id_class_map[box]:
  ...
  for box, color in box_to_color_map.items():
    ...
    display_str = box_to_display_str_map[box][0]
    
    if 'Fish' in display_str:
        NFish=np.concatenate( ( NFish, [[round(xmin*image.shape[1]), round(ymin*image.shape[0]), round(xmax*image.shape[1]), round(ymax*image.shape[0])]] ))
    if 'Fish' in display_str:
        NAll_list= np.concatenate((NAll_list, [['Fish', round(xmin*image.shape[1]), round(ymin*image.shape[0]), round(xmax*image.shape[1]), round(ymax*image.shape[0])]]))
        print("", display_str,'xmin:',round(xmin*image.shape[1]),'ymin:',round(ymin*image.shape[0]),'xmax:',round(xmax*image.shape[1]),'ymax:',round(ymax*image.shape[0]))

    NAll= np.concatenate( ( NAll, [[display_str,round(xmin*image.shape[1]), round(ymin*image.shape[0]), round(xmax*image.shape[1]), round(ymax*image.shape[0])]] ))               
    N=np.concatenate( ( N, [[round(xmin*image.shape[1]), round(ymin*image.shape[0]), round(xmax*image.shape[1]), round(ymax*image.shape[0])]] ))
    ...
    elif 'Fish' in box_to_id_class_map[box]:
        draw_bounding_box_on_image_array(
          image,
          ymin,
          xmin,
  ...
  return image,N,NFish,NAll,NAll_list
